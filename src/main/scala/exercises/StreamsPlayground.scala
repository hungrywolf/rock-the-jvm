...
class ConsStream[+A](hd: A, tl: => MyStream[A]) extends MyStream[A] {
  ...
  /*
    ...
    mapped = s.map(_ + 1) = new Cons(2, s.tail.map(_ + 1))
      ... mapped.tail
   */
  def map[B](f: A => B): MyStream[B] = new ConsStream(f(head), tail.map(f)) // preserves lazy evaluation
...
object StreamsPlayground extends App{
  ...
  def eratosthenes(numbers: MyStream[Int]): MyStream[Int] =
    ...
    else new ConsStream(numbers.head, eratosthenes(numbers.tail.filter(_ % numbers.head != 0)))
